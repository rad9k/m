...
- **Back-end**
	...
	As -zero is not capable yet to serve web front-ends on its own, we position -zero as a back-end platform, exposing REST services. Here it can shine storing and exposing any data alongside with logic. The logic lives in the graph database thus there is no impedence mismatch or need for any data ⇔ code translation layer like ORM. The data structures schema can be designed with diagram editor. Back-end logic can be expressed by diagrams, code or any desired mix of the two (in fact you can have code and diagrams at once on the screen).
