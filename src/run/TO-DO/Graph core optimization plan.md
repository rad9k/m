...
- **Domknięcie watch itemu cross-store detach/attach — zrealizowane 2026-07-16.** Diagnostyka faz wykazała liniowe skalowanie; przy 1000 edges `Detach` alokował `64,071 B`, a `Attach` `376,010 B`. Źródłem alokacji attach były dwa wykonywane dla każdej krawędzi `MinusZero.GetStore()` oparte na `Where(...).FirstOrDefault()` z przechwyconą lambdą. Równoważna pętla indeksowa nie alokuje; przy 2 store lookup spada z `57.49 ns`, `136 B` do `27.89 ns`, `0 B`, a przy 32 store zachowuje ten sam czas w granicach dyspersji (`500.06 ns` kontra `504.57 ns`). Porównywalny `ShortRun` pełnego detach+attach daje teraz `231.4 ns`, `22.713 us`, `444.666 us` oraz `168 B`, `10,464 B`, `104,068 B` dla 1/100/1000 edges. Względem correction-3 `ShortRun` czas spada o `31.5%/27.6%/9.3%`, a względem bezpośredniego rebaseline o `36.1%/35.3%/20.7%`; alokacja spada o `66.7%/76.3%/76.4%`. Dodany kontrakt potwierdza, że oba overloady lookupu zwracają istniejącą instancję bez duplikacji store.
